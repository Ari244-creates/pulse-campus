...
exports.getEvents = (req, res) => {
    db.all('SELECT id, name, priority, space_id, start_time, end_time FROM events', [], (err, rows) => {
...
exports.evaluateResource = async (req, res) => {
    ...
    db.get(
        'SELECT e.*, s.capacity, s.name as space_name FROM events e JOIN spaces s ON e.space_id = s.id WHERE e.id = ?',
        [event_id],
        async (err, event) => {
            ...
            try {
                ...
                // Conflict detected, find alternative
                const alternative = await findAlternativeSpace(event.space_id, event.capacity, event.start_time);
                ...
                // Logic assumed: REASSIGNED response comes back after evaluation if a better fit is found
                ...
                // We automatically reassign in this logic for the "REASSIGNED" response requirement
                db.serialize(() => {
                    db.run('BEGIN TRANSACTION');
                    db.run('UPDATE events SET space_id = ? WHERE id = ?', [alternative.id, event_id]);
                    db.run(
                        'INSERT INTO reassignment_logs (event_id, from_space_id, to_space_id, reason) VALUES (?, ?, ?, ?)',
                        [event_id, event.space_id, alternative.id, `Predicted overload (${utilization}%)`]
                    );
                    db.run('COMMIT', (err) => {
                        if (err) return res.status(500).json({ error: err.message });
...
exports.reassignResource = (req, res) => {
    ...
    db.get('SELECT space_id FROM events WHERE id = ?', [event_id], (err, event) => {
        if (err || !event) return res.status(404).json({ error: 'Event not found' });
        ...
        db.serialize(() => {
            db.run('BEGIN TRANSACTION');
            db.run('UPDATE events SET space_id = ? WHERE id = ?', [to_space_id, event_id]);
            db.run(
                'INSERT INTO reassignment_logs (event_id, from_space_id, to_space_id, reason) VALUES (?, ?, ?, ?)',
            ...
            db.run('COMMIT', (err) => {
                if (err) return res.status(500).json({ error: err.message });
                res.json({ status: 'REASSIGNED', event_id, from_space_id, to_space_id, reason });
            });
        });
    });
};
...
exports.getDecisionHistory = (req, res) => {
    ...
    JOIN spaces s1 ON l.from_space_id = s1.id
    JOIN spaces s2 ON l.to_space_id = s2.id
